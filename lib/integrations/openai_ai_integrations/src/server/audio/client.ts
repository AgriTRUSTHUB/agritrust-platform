...
interface AudioMessageWithAudio {
  audio?: { transcript?: string; data?: string };
  content?: string;
}

interface AudioDeltaWithAudio {
  audio?: { transcript?: string; data?: string };
}
...
export async function voiceChat(
  ...
  const message = response.choices[0]?.message as unknown as AudioMessageWithAudio;
...
export async function voiceChatStream(
  ...
  return (async function* () {
    for await (const chunk of stream) {
      const delta = chunk.choices?.[0]?.delta as unknown as AudioDeltaWithAudio;
      if (!delta) continue;
...
export async function textToSpeech(
  ...
  const audioData = (response.choices[0]?.message as unknown as AudioMessageWithAudio)?.audio?.data ?? "";
